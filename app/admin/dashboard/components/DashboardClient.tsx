...
import { crearTrabajador, resetearDispositivo, eliminarTrabajador, editarTrabajador, guardarConfiguracion, crearAsistenciaManual, activarTrabajador } from '@/actions/admin'
import { Loader2, RefreshCcw, Smartphone, UserPlus, Pencil, Trash2, Settings, AlertTriangle, ChevronUp, ChevronDown, Clock, Eye, EyeOff, Search, PenSquare, Users, Lock, UserCheck } from 'lucide-react'
    ...
    <ClientPortal>
      <div className="fixed inset-0 bg-black/60 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm">
        <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 animate-in fade-in zoom-in-95 duration-200">
          ...
          <h3 className="font-bold text-xl text-gray-900 mb-1.5">Dar de baja a Trabajador</h3>
          <p className="text-gray-600 text-xs mb-4 leading-relaxed">
            ¿Estás seguro de dar de baja a <strong className="text-slate-900">{workerName}</strong>? Esta acción pasará al trabajador a estado <strong>inactivo</strong>, liberará sus llaves de dispositivo y conservará su historial de asistencias intacto según las exigencias de SUNAFIL.
          </p>
...
function EditWorkerModal({ isOpen, worker, onClose, onSave, onActivate, isPending }: {
  isOpen: boolean
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center justify-between gap-2">
              <h3 className="font-bold text-xl text-gray-900 truncate">Editar Trabajador</h3>
              {worker.activo === false && (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-between gap-3 pt-2">
            {worker.activo === false && onActivate ? (
              <button
                type="button"
                onClick={onActivate}
                disabled={isPending}
                className="flex items-center gap-1.5 px-4 py-2.5 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl transition-colors shadow-md shadow-emerald-600/20 disabled:opacity-50"
              >
...
export function WorkerActions({ 
  ...
  const handleActivate = () => {
    startTransition(async () => {
      const res = await activarTrabajador(id)
      if (res.error) toast.error(res.error)
      if (res.success) {
        toast.success(res.success)
        setModalState({type: null})
  ...
  return (
    <>
      {isActivo ? (
        ...
        <div className="flex items-center gap-2 mt-2">
          <button
            ...
            disabled={isPending}
            className="text-[11px] flex flex-1 items-center justify-center gap-1.5 px-2.5 py-1.5 rounded-lg font-bold text-emerald-800 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 transition-colors disabled:opacity-50"
            title="Reactivar trabajador en caso de recontratación"
          >
            {isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <UserCheck className="w-3.5 h-3.5 text-emerald-600" />}
            <span>Activar</span>
          </button>
      ...
      <EditWorkerModal
        ...
        worker={modalState.type === 'edit' ? { id, nombre: currentName, usuario: currentUsuario, activo: isActivo !== false, horarioEspecial: currentHorarioEspecial } : null}
        onClose={() => setModalState({ type: null })}
        onSave={handleEdit}
        onActivate={isActivo === false ? handleActivate : undefined}
        isPending={isPending}
      />
...
export function DateFilter({ workerId }: { workerId?: string }) {
  ...
  // Calcular diferencia en días (rango máximo 90 días, tolerancia de seguridad 92 días)
  const calcularExcesoDias = () => {
    if (!desde || !hasta) return false
    const dInicio = new Date(`${desde}T00:00:00`)
    const dFin = new Date(`${hasta}T23:59:59.999`)
    if (isNaN(dInicio.getTime()) || isNaN(dFin.getTime())) return false
    const diffTime = Math.abs(dFin.getTime() - dInicio.getTime())
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24))
    return diffDays > 92
  }

  const isRangoExcedido = calcularExcesoDias()

  const aplicarFiltro = () => {
    if (isRangoExcedido) {
      toast.error('El rango máximo permitido para exportar o consultar es de 3 meses (90 días) para proteger el rendimiento')
      return
    }
  ...
  return (
    <div className="flex flex-col gap-2 w-full max-w-xl min-w-[340px]">
      {/* Alerta visual si se superan los 92 días */}
      {isRangoExcedido && (
        <div className="bg-amber-50 border border-amber-300 text-amber-900 text-xs px-3 py-2 rounded-xl flex items-center gap-2 animate-in fade-in duration-200 shadow-xs">
          <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0" />
          <span className="font-semibold">
            El rango máximo permitido para exportar o consultar es de 3 meses (90 días) para proteger el rendimiento.
          </span>
        </div>
      )}
      ...
      <div className="flex items-center justify-between px-1">
        ...
        {isRangoExcedido ? (
          <button 
            type="button"
            onClick={() => toast.error('El rango máximo permitido para exportar o consultar es de 3 meses (90 días) para proteger el rendimiento')}
            className="text-xs flex items-center gap-1 bg-gray-100 text-gray-400 px-3 py-1.5 rounded-lg font-bold border border-gray-200 h-7 whitespace-nowrap shrink-0 opacity-60 cursor-not-allowed"
            title="El rango seleccionado supera el límite máximo de 90 días"
          >
      ...
      <p className="text-[11px] text-gray-400 px-1">
        ℹ️ No se puede consultar más de tres meses, si lo requiere consulte a soporte Técnico.
      </p>
    </div>
  )
}
...
export function WorkerListSearch({ 
  ...
  const isCurrentWorkerInactive = inactivos.some(t => t.id === workerId)

  const q = query.toLowerCase().trim()
  const activosFiltrados = activos.filter(t => 
    !q || (t.nombre_completo || '').toLowerCase().includes(q) || (t.usuario || '').toLowerCase().includes(q)
  )
  const inactivosFiltrados = inactivos.filter(t => 
    !q || (t.nombre_completo || '').toLowerCase().includes(q) || (t.usuario || '').toLowerCase().includes(q)
  )

  const totalMostrados = activosFiltrados.length + (query || mostrarInactivos || isCurrentWorkerInactive ? inactivosFiltrados.length : 0)

  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col max-h-[560px]">
      {/* Header con contador */}
      <div className="p-4 bg-gray-50 border-b border-gray-200 flex items-center justify-between gap-2 shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <h3 className="font-bold text-gray-800 truncate">Trabajadores</h3>
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="bg-blue-100 text-blue-700 text-xs font-bold px-2 py-0.5 rounded-full" title="Trabajadores activos">
            {activosFiltrados.length} Activos
          </span>
          {inactivos.length > 0 && (
            <span className="bg-slate-100 text-slate-600 text-xs font-semibold px-2 py-0.5 rounded-full" title="Trabajadores inactivos">
      ...
      <ul className="divide-y divide-gray-100 overflow-y-auto overflow-x-hidden custom-scrollbar flex-1">
        {totalMostrados === 0 ? (
          ...
          <>
            ...
            {/* SECCIÓN DESPLEGABLE DE TRABAJADORES INACTIVOS */}
            {inactivos.length > 0 && !query && (
              <li className="bg-slate-50/90 border-t border-gray-200">
                <button
                  type="button"
                  onClick={() => setMostrarInactivos(!mostrarInactivos)}
                  className="w-full flex items-center justify-between text-xs font-semibold text-slate-600 hover:text-slate-900 transition-colors p-3 hover:bg-slate-100 cursor-pointer"
                >
                  <span className="flex items-center gap-2">
                    <Users className="w-3.5 h-3.5 text-slate-400" />
                    <span>Personal Inactivo / Bajas ({inactivos.length})</span>
                  </span>
                  <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${(mostrarInactivos || isCurrentWorkerInactive) ? 'rotate-180 text-blue-600' : ''}`} />
                </button>
              </li>
            )}

            {/* LISTA DE TRABAJADORES INACTIVOS (Visible al expandir o al buscar o si está seleccionado) */}
            {(mostrarInactivos || isCurrentWorkerInactive || query) && inactivosFiltrados.map((t) => (
              <li 
                key={t.id} 
                className={`p-3.5 transition-colors ${
                  workerId === t.id 
                    ? 'bg-amber-50/80 border-l-4 border-amber-500' 
                    : 'bg-slate-50/50 hover:bg-slate-100/70 border-l-4 border-transparent'
                }`}
              >
                <div className="flex flex-col gap-1.5 opacity-90">
                  <Link href={`?workerId=${t.id}`} className="flex items-center gap-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-1">
                        <span className="font-semibold text-gray-700 text-sm truncate block hover:text-blue-600 transition-colors cursor-pointer">
                          {t.nombre_completo}
                        </span>
                        <span className="text-[10px] font-bold text-slate-500 bg-slate-200/80 px-1.5 py-0.5 rounded shrink-0">
                          Inactivo
                        </span>
                      </div>
                      <span className="block text-xs text-gray-400">@{t.usuario}</span>
                    </div>
                  </Link>
